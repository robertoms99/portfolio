...
const handleScrollTopSection = () => {
  ...
  window.requestAnimationFrame(() => {
    for (const caretElement of caretElements as any) {
      if (
        !(caretElement.matches(
          `.${DOM_STRING.caretStarting}, .${DOM_STRING.caretFinished}`
        ) as boolean) &&
        isVisible(caretElement)
      ) {
...
const typeCaretAnimation = (caretElement: Element) => {
  ...
  const intervalId = setInterval(() => {
    caretElement.textContent = `${caretElement.textContent ?? ''}${
      (caretElementContent[index] as string) ?? ''
    }`
    index++
